...
# 데이터프레임 생성
basis_df = pd.DataFrame(data, index=['1', '2'])
...
for idx, df in enumerate(dataframes): #cif파일 하나씩 처리하고 반복 (기저벡터와 유사한 plane spacing 찾기)
    ...
    for index, row in basis_df.iterrows(): #주어진 basis dataframe 에 대해 반복하여 기저벡터 길이와 유사한 plane spacing group을 찾기
        ...
        given_d = basis_df['d(Å)'][index]
        
        df['d Error']=abs(df['d(Å)']-given_d)/df['d(Å)']
        df_sorted_unique = df.drop_duplicates('d Error').sort_values(by='d Error').reset_index(drop=True)
        if df_sorted_unique['d Error'].iloc[0]>0.03:
            selected_group_temp=df[df['d(Å)']==df_sorted_unique['d(Å)'].iloc[0]]
        else:
            selected_group_temp=df[df['d Error']<=0.03]
        
        print (selected_group_temp)   
        min_d_error=df_sorted_unique['d Error'].iloc[0]
        #phase 간의 정확성 비교 위해 오차의 총합 구하기
        min_error += min_d_error


        # 선택된 그룹을 새로운 DataFrame으로 만들어 저장 (이름과 함께)
        selected_group_name = f"{index}_group - ({df.name})"
        selected_group = selected_group_temp
    ...
    for i1 in range(n_rows_group1):
        for i2 in range(n_rows_group2):
            angle,hkl1,hkl2 = get_angle(df.name, g_groups[0], i1, g_groups[1], i2)  # get angle 함수 사용해서 두 그룹 내 하나씩 반복해서 각도를 구함
            ang_err = abs(basis_ang - angle)  # 오차 반영을 위해 각도 오차를 계산
    ...
    w=h1*k2-h2*k1
    zoneaxis=[u,v,w]
    print('Zone Axis: ',zoneaxis)
    print('\n')
    name_list.append(name)
    d_err_list.append(min_error)
    ang_err_list.append(min_ang_err)
    err_sum = min_error + min_ang_err
    err_list.append(err_sum)

data={
    'Name':name_list,
    'd Error':d_err_list,
    'Ang Error':ang_err_list,
    'Error Sum':err_list
    }
err_df=pd.DataFrame(data)
sorted_err_df=err_df.sort_values(by='Error Sum', ascending=False).reset_index(drop=True)

# 결과 출력
print(sorted_err_df, '\n')

# df_final=df[df.name==sorted_err_df['Name'].iloc[0]]
